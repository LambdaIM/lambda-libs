...
func TestMethodSignature(t *testing.T) {
	...
	m := Method{"foo", false, []Argument{{"bar", String, false, nil}, {"baz", String, false, nil}}, nil}
	...
	m = Method{"foo", false, []Argument{{"bar", uintt, false, nil}}, nil}
	exp = "foo(uint256)"
	if m.Sig() != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig())
	}
...
func TestEncode(t *testing.T) {
	abiJson := `
	[{"constant":false,"inputs":[{"name":"addr1","type":"address"},{"name":"name1","type":"bytes32"},{"name":"name2","type":"bytes20"},{"name":"name3","type":"bytes"},{"name":"name4","type":"string"},{"name":"num1","type":"uint256"},{"name":"num2","type":"uint256[]"},{"name":"num3","type":"int256"},{"name":"num4","type":"int256[3]"},{"name":"num5","type":"int128"},{"name":"num6","type":"int128[2]"},{"name":"num8","type":"int256"},{"name":"bool1","type":"bool"}],"name":"typeEncode","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"}]
	`
	abi, err := JSON(strings.NewReader(abiJson))
	if err != nil {
		t.Fatal(err)
	...
	args := "addr1, bytes1aaaaa, bytes2bbbbb, bytes3ccccc, string1, 1, [2,2,2,2], 3, [4,4,4], 5, [6,6], 7, false"
	encoded, err := abi.Encode("typeEncode", args)
	if err != nil {
		t.Fatal(err)
	}
	t.Log(common.Bytes2Hex(encoded))
}

func print_output(m interface{}, t *testing.T) {
	switch ty := m.(type) {
	case [][]interface{}:
	...
	case []byte:
		t.Log(common.Bytes2Hex(ty))
	default:
		t.Log(ty)
	}
}

func TestDecodeTupleList(t *testing.T) {
	abiJson := `
		[{"constant":true,"inputs":[{"name":"","type":"uint256"}],"name":"PledgeMinerList","outputs":[{"name":"size","type":"uint256"},{"name":"useSize","type":"uint256"},{"name":"addr1","type":"bytes"},{"name":"status","type":"uint256"},{"name":"pledgeTime","type":"uint256"}],"payable":false,"stateMutability":"view","type":"function"},{"constant":false,"inputs":[],"name":"getMiner","outputs":[{"components":[{"name":"size","type":"uint256"},{"name":"useSize","type":"uint256"},{"name":"addr1","type":"bytes"},{"name":"status","type":"uint256"},{"name":"pledgeTime","type":"uint256"}],"name":"","type":"tuple"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[],"name":"getPledgeMinerList","outputs":[{"components":[{"name":"size","type":"uint256"},{"name":"useSize","type":"uint256"},{"name":"addr1","type":"bytes"},{"name":"status","type":"uint256"},{"name":"pledgeTime","type":"uint256"}],"name":"","type":"tuple[]"}],"payable":false,"stateMutability":"nonpayable","type":"function"}]
	`
	abi, err := JSON(strings.NewReader(abiJson))
	if err != nil {
		t.Fatal(err)
	...
	decoded, err := abi.Decode("getPledgeMinerList", common.Hex2Bytes(""))
	if err != nil {
		t.Fatal(err)
	}
	print_output(decoded, t)
}
